...
describe("Samizdat Program – Happy Path", () => {
    ...
    before(async () => {
        ...
        const publisherKeypairBytes = new Uint8Array(
            JSON.parse(
                readFileSync(`${process.env.HOME}/.config/solana/devnet-test.json`, "utf-8"),
            ),
        );
        ...
        const operatorKeypairBytes = new Uint8Array(
            JSON.parse(
                readFileSync(`${process.env.HOME}/.config/solana/devnet-operator.json`, "utf-8"),
            ),
        );
        operator = await createKeyPairSignerFromBytes(operatorKeypairBytes);

        ({ pda: publisherAccountPDA } = await getPDAAndBump(SAMIZDAT_PROGRAM_ADDRESS, [
            "publisher",
            publisher.address,
        ]));

        ({ pda: campaignAccountPDA } = await getPDAAndBump(SAMIZDAT_PROGRAM_ADDRESS, [
            "campaign",
            publisherAccountPDA,
            CAMPAIGN_ID,
        ]));
        ...
        const pub = await fetchPublisherAccount(connection.rpc, publisherAccountPDA);
    ...
    describe("Publisher Registration", () => {
        test("registers a publisher account", async () => {
            ...
            const account = await fetchPublisherAccount(connection.rpc, publisherAccountPDA);
    ...
    describe("Campaign Lifecycle", () => {
        test("creates a campaign with upfront funding", async () => {
            ...
            const campaign = await fetchCampaignAccount(connection.rpc, campaignAccountPDA);
    ...
    describe("Play Cycle - Claim & Confirm", () => {
        test("operator claims a campaign", async () => {
            ...
            const cooldown = await fetchClaimCooldown(connection.rpc, claimCooldownPDA);
            ...
            const campaignAfter = await fetchCampaignAccount(connection.rpc, campaignAccountPDA);
            assert.strictEqual(
        ...
        test("publisher confirms the play and pays the node", async () => {
            ...
            const pub = await fetchPublisherAccount(connection.rpc, publisherAccountPDA);
            assert.strictEqual(pub.data.totalSpent, initialTotalSpent + BOUNTY_PER_PLAY);
        });
    });
    ...
    describe("Timeout Play", () => {
        ...
        before(async () => {
            ({ pda: timeoutPlayRecordPDA } = await getPDAAndBump(SAMIZDAT_PROGRAM_ADDRESS, [
                "play_record",
                campaignAccountPDA,
                nodeAccountPDA,
                TIMEOUT_NONCE,
    ...
    describe("Close Campaign", () => {
        ...
        before(async () => {
            ({ pda: closeCampaignPDA } = await getPDAAndBump(SAMIZDAT_PROGRAM_ADDRESS, [
                "campaign",
                publisherAccountPDA,
                CLOSE_CAMPAIGN_ID,
            ]));
        ...
        test("closes a campaign and reclaims rent + remaining funds", async () => {
            const publisherBalanceBefore = await connection.getLamportBalance(publisher.address);
            ...
            const publisherBalanceAfter = await connection.getLamportBalance(publisher.address);
